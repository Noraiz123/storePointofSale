        ...
        <div className='inline-block w-auto p-6 my-8 text-left align-middle transition-all transform bg-white shadow-2xl rounded-2xl'>
          ...
          <div className='mt-10 h-30v overflow-y-auto'>
            ...
            <div className='flex space-x-2 justify-center items-center my-2'>
              ...
              />
              <label for='payment-checkbox' className='mb-1 text-gray-500 font-bold'>
                Send to cashflow
              </label>
            </div>
          </div>
          <div className='mt-4 space-x-2 flex items-end h-auto'>
            {(Number(payment) >= Math.round(totalPrice) || cashflow) && (
              <button
                type='button'
                onClick={() => {
                  confirmOrder(Math.round(totalPrice - Number(payment)), Number(payment), cashflow);
                  setPayment('');
                }}
