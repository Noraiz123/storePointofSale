...
export default (state = initialState.cashFlow, action) => {
  switch (action.type) {
    case actionTypes.getCashflow: {
      const { cashflow, currentPage, totalPages, noOfCashflow, totalCashflow, cashflowStats } = action.payload;
      return { ...state, cashflow, currentPage, totalPages, noOfCashflow, totalCashflow, cashflowStats };
    }
