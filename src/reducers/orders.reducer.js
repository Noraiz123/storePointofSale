  ...
  switch (action.type) {
    ...
    case actionTypes.createOrder: {
      ...
      if (action.status === 'CREATE_ORDER') {
        alreadyExists = state.currentOrder.find(
          (e) => e._id === action.payload._id && e.variated === action.payload.variated
        );
        if (alreadyExists) {
          updateItem =
            alreadyExists &&
            state.currentOrder.map((e) =>
              e._id === action.payload._id && e.variated === action.payload.variated
